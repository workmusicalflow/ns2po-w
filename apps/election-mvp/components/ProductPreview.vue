    ...
    <div class="customization-panel">
      ...
      <div class="customization-section">
        ...
        <div v-if="customization.logos?.length" class="logos-list">
          ...
          >
            <img :src="logo.url" :alt="`Logo ${index + 1}`" class="logo-preview">
            <div class="logo-controls">
              ...
              <input 
                ...
                max="200"
                class="control-slider"
                @input="updateLogo(index, { width: Number($event.target.value), height: Number($event.target.value) })"
              >
      ...
      <div class="price-section">
        ...
        <div class="price-breakdown">
          <div class="price-line">
            ...
            <span class="price-value">{{ formatCurrency(props.product.basePrice) }}</span>
          </div>
          
          <div v-if="priceCalculation.customizationsCost > 0" class="price-line">
            <span class="price-label">Personnalisations :</span>
            <span class="price-value">{{ formatCurrency(priceCalculation.customizationsCost) }}</span>
          </div>
          
          <div v-if="priceCalculation.appliedRules.length > 0" class="price-line">
            ...
            <div class="rules-list">
              ...
              >
                ...
                <span :class="['rule-value', rule.modifier < 0 ? 'text-green-600' : 'text-orange-600']">
          ...
          <div class="price-line price-line--total">
            ...
            <span class="price-value price-value--highlight">{{ formatCurrency(priceCalculation.unitPrice) }}</span>
          </div>
          
          <div class="price-line price-line--total">
            <span class="price-label">Total ({{ quantity }} unités) :</span>
            <span class="price-value price-value--total">{{ formatCurrency(priceCalculation.totalPrice) }}</span>
          </div>
        </div>
      </div>
...
const priceCalculation = computed(() => {
  ...
  if (customization.value.logos?.length) {
    customizations.push({ priceModifier: customization.value.logos.length * 500 })
  }
  ...
  if (customization.value.colors?.length) {
    customizations.push({ priceModifier: customization.value.colors.length * 200 })
  }
...
const removeLogo = (index: number) => {
  ...
  customization.value.logos = customization.value.logos.filter((_, i) => i !== index)
  
  if (selectedLogoIndex.value === index) {
    selectedLogoIndex.value = null
  } else if (selectedLogoIndex.value !== null && selectedLogoIndex.value > index) {
...
const updatePreview = async () => {
  ...
  try {
    ...
    if (customization.value.text) {
      ...
      ctx.font = isMobile.value ? 'bold 14px sans-serif' : 'bold 18px sans-serif'
    ...
    uploadError.value = 'Erreur lors de la prévisualisation. Veuillez réessayer.'
    setTimeout(() => {
      uploadError.value = null
    }, 3000)
  }
...
const renderCanvasContent = async (ctx: CanvasRenderingContext2D, width: number, height: number) => {
  // Fond du produit
  if (productImageCache.value) {
    ctx.drawImage(productImageCache.value, 0, 0, width, height)
  } else {
    ctx.fillStyle = '#f3f4f6'
  ...
  if (customization.value.colors.length > 0) {
    const color = availableColors.find(c => c.value === customization.value.colors[0])
  ...
  // Logos
  for (const [index, logo] of customization.value.logos.entries()) {
    if (logoImageCache.value[index]) {
      ctx.save()
      ...
      ctx.drawImage(logoImageCache.value[index], logo.x, logo.y, logo.width, logo.height)
      ctx.restore()
    }
  }
