<template>
  ...
  >
    ...
    <div class="relative h-full overflow-hidden">
      <NuxtImg
        v-if="realisation.cloudinaryUrls && realisation.cloudinaryUrls.length > 0"
      ...
      >
        <svg class="h-12 w-12 text-primary/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    >
      ...
      >
        ...
        <span>{{ realisation.productIds.length }} produit{{ realisation.productIds.length > 1 ? 's' : '' }}</span>
      </div>
    </div>
...
const handleImageClick = () => {
  if (realisation.value.cloudinaryUrls && realisation.value.cloudinaryUrls.length > 0) {
...
watch(() => realisation.value, (newRealisation) => {
  if (newRealisation) {
