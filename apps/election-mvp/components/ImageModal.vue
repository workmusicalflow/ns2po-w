  ...
  <Teleport to="body">
    ...
    >
      ...
      >
        <!-- Container modal -->
        <div
          class="relative max-w-7xl max-h-full w-full"
          @click.stop
        >
          <!-- Bouton fermeture -->
          <button
          ...
          >
            <div class="animate-pulse flex space-x-4">
              <div class="rounded-full bg-primary/20 h-10 w-10"></div>
              <div class="flex-1 space-y-6 py-1">
                <div class="h-2 bg-primary/20 rounded"></div>
                <div class="space-y-3">
                  <div class="grid grid-cols-3 gap-4">
                    <div class="h-2 bg-primary/20 rounded col-span-2"></div>
                    <div class="h-2 bg-primary/20 rounded col-span-1"></div>
                  </div>
                  <div class="h-2 bg-primary/20 rounded"></div>
                </div>
              </div>
        ...
        <div class="absolute bottom-6 left-1/2 transform -translate-x-1/2 text-white/70 text-sm text-center">
          <p>Cliquez en dehors ou appuyez sur <kbd class="px-2 py-1 bg-white/20 rounded">Échap</kbd> pour fermer</p>
        </div>
      </div>
    </Transition>
  </Teleport>
...
const { isOpen, imageUrl, imageAlt, realisationTitle, closeModal } = useImageModal()
