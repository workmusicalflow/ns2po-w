 ...
 */
export function calculateOptimalSizes(context: 'product' | 'gallery' | 'thumbnail' | 'hero' | 'custom', customSizes?: string): string {
  const sizeMap = {
...
export function buildCompleteResponsiveConfig(
  ...
  const breakpoints = responsiveBreakpoints[context] || responsiveBreakpoints.default
  const sources = buildResponsiveImageSourcesWithSrcSet(publicId, options, breakpoints)
