...
export type ThemeVariant = 'default' | 'election-primary' | 'election-secondary' | 'dark'
...
export const useTheme = () => {
  ...
  const loadThemeFromStorage = () => {
    if (process.client) {
      ...
      const savedColorScheme = localStorage.getItem('ns2po-color-scheme') as ColorScheme
...
export const initializeTheme = () => {
  if (process.client) {
    const { loadThemeFromStorage, detectSystemColorScheme, applyThemeToDOM } = useTheme()
