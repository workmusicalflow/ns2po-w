...
export const useContactForm = () => {
  ...
  const lastSubmission = ref<
    ContactSubmissionResponse | PreorderSubmissionResponse | null
  >(null);
  ...
  const validatePreorderForm = (
    formData: Partial<PreorderFormData>
  ): FormValidationResult => {
    const errors: FormFieldError[] = [];
  ...
  const submitPreorderForm = async (
  ...
  ): Promise<PreorderSubmissionResponse> => {
    isSubmitting.value = true;
    validationErrors.value = [];
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Erreur lors de l'enregistrement de la pré-commande";
      throw new Error(errorMessage);
    } finally {
      isSubmitting.value = false;
    }
