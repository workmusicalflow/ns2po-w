...
test.describe('Parcours Inspiration → Conversion', () => {
  ...
  test('Parcours complet : Homepage → Réalisation → Inspiration → Produit → Devis', async () => {
    ...
    await test.step('Cliquer sur une réalisation depuis la homepage', async () => {
      ...
      const realisationCard = await waitForClickableElement(page, TestSelectors.REALISATION_CARD, TestTimeouts.NETWORK_LOAD)
      
      // Cliquer sur le bouton "S'inspirer"
      const inspireButton = realisationCard.locator(TestSelectors.REALISATION_INSPIRE_BUTTON)
    ...
    await test.step('Vérifier la redirection vers la page produit avec contexte', async () => {
      ...
      const inspirationBanner = page.locator('[data-testid="inspiration-banner"]')
    ...
    await test.step('Personnaliser le produit avec un logo', async () => {
      ...
      const customizationTool = page.locator('[data-testid="product-customizer"]')
      ...
      if (await logoUpload.count() > 0) {
        ...
        await page.waitForSelector('[data-testid="preview-canvas"]', { timeout: 15000 })
      }
    })
    ...
    await test.step('Modifier la quantité et vérifier le prix', async () => {
      ...
      const quantityInput = page.locator('input[data-testid="quantity-input"], select[data-testid="quantity-select"]')
      ...
      await page.waitForFunction(() => {
        ...
        return priceElement && priceElement.textContent && priceElement.textContent !== '0'
      })
    })

    // 5. Ajouter au devis
    await test.step('Ajouter le produit au devis', async () => {
      const addToQuoteButton = page.locator('button:has-text("Ajouter au devis"), button[data-testid="add-to-quote"]')
      ...
      const successMessage = page.locator('.notification, [data-testid="success-message"], .toast')
      await expect(successMessage).toBeVisible({ timeout: 5000 })
    })

    // 6. Accéder au panier/devis
    await test.step('Accéder au panier de devis', async () => {
      const cartButton = page.locator('button:has-text("Panier"), button:has-text("Devis"), [data-testid="cart-button"]')
    ...
    await test.step('Remplir le formulaire de demande de devis', async () => {
      ...
      const quoteForm = page.locator('form[data-testid="quote-form"], form:has(input[name="customerName"])')
      await expect(quoteForm).toBeVisible()
      
      // Remplir les informations client
      await page.fill('input[name="customerName"], input[placeholder*="nom"]', 'Jean Kouassi')
      await page.fill('input[name="email"], input[type="email"]', 'jean.kouassi@example.com')
      await page.fill('input[name="phone"], input[type="tel"]', '+225 01 02 03 04 05')
      
      // Sélectionner le contexte électoral
      const contextSelect = page.locator('select[name="context"], select:has(option:text-matches("électoral", "i"))')
      ...
      if (await messageField.count() > 0) {
        await messageField.fill('Devis pour campagne électorale municipale. Besoin urgent.')
      }
    })

    // 8. Soumettre le devis et vérifier la conversion
    await test.step('Soumettre le devis et vérifier la conversion', async () => {
      const submitButton = page.locator('button[type="submit"]:has-text("Envoyer"), button:has-text("Demander un devis")')
  ...
  test('Parcours inspiration avec produit indisponible', async () => {
    await test.step('Simuler un produit temporairement indisponible', async () => {
      ...
      const realisationCard = page.locator('[data-testid="realisation-card"]').first()
      await realisationCard.locator('[title="S\'inspirer de cette réalisation"]').click()
      
      // Vérifier que l'interface gère correctement l'indisponibilité
      const unavailableMessage = page.locator(':text("indisponible"), [data-testid="product-unavailable"]')
      await expect(unavailableMessage).toBeVisible()
      
      // Vérifier qu'un produit alternatif est proposé
      const alternativeProduct = page.locator('[data-testid="alternative-product"], :text("produit similaire")')
      await expect(alternativeProduct).toBeVisible()
    })
  })

  test('Performance du parcours inspiration sur connexion lente', async ({ page, browserName }) => {
    ...
    await test.step('Mesurer les performances du parcours complet', async () => {
      ...
      await page.goto('/')
      await page.waitForSelector('[data-testid="realisation-card"]', { timeout: 15000 })
      ...
      await page.locator('[data-testid="realisation-card"]').first()
        .locator('[title="S\'inspirer de cette réalisation"]').click()
  ...
  test('Tracking GA4 du parcours inspiration-conversion', async () => {
    ...
    await test.step('Vérifier le tracking des événements GA4', async () => {
      ...
      const realisationLogs = debugLogs.filter((log: any) => 
        log.module === 'RealisationTracking' || log.module === 'ConversionTracking'
      )
  ...
  test('Gestion d\'erreur lors de la soumission de devis', async () => {
    await test.step('Simuler une erreur de soumission de devis', async () => {
      ...
      await page.goto('/')
      await page.waitForSelector('[data-testid="realisation-card"]', { timeout: 10000 })
      ...
      await page.fill('input[name="customerName"], input[placeholder*="nom"]', 'Test User')
      ...
      const errorMessage = page.locator('.error, [data-testid="error-message"], .notification-error')
