...
test.describe('Realisation Cards Functionality', () => {
  test('Should display realisation cards and handle inspiration click', async ({ page }) => {
    // Go to homepage
    ...
    await page.screenshot({ path: 'test-results/realisation-cards-loaded.png' })
    
    // Click on the inspiration button of the first card
    const firstCard = realisationCards.first()
    const inspirationButton = firstCard.locator('[title="S\'inspirer de cette réalisation"]')
    
    // Wait for the button to be visible and clickable
    await inspirationButton.waitFor({ state: 'visible', timeout: 10000 })
    await inspirationButton.click()
    
    // Check that we navigated to catalogue with inspiration parameters
    await page.waitForURL(/\/catalogue.*inspiredBy=/, { timeout: 10000 })
    ...
    // Verify the catalogue page loads
