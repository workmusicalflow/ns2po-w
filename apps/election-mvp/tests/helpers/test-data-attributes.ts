...
export const TestSelectors = {
  ...
  QUANTITY_INPUT: 'input[data-testid="quantity-input"], select[data-testid="quantity-select"]',
  TOTAL_PRICE: '[data-testid="total-price"]',
  ADD_TO_QUOTE_BUTTON: 'button[data-testid="add-to-quote"], button:has-text("Ajouter au devis")',
  
  // Panier / Devis
  CART_BUTTON: '[data-testid="cart-button"], button:has-text("Panier"), button:has-text("Devis")',
  QUOTE_FORM: 'form[data-testid="quote-form"], form:has(input[name="customerName"])',
  ...
  SUCCESS_CONFIRMATION: '[data-testid="success-confirmation"], :text("devis envoyé"), :text("demande reçue"), :text("merci")',
  
  // Produits indisponibles
  PRODUCT_UNAVAILABLE: '[data-testid="product-unavailable"], :text("indisponible")',
  ALTERNATIVE_PRODUCT: '[data-testid="alternative-product"], :text("produit similaire")',
} as const
...
export const FormFields = {
  ...
  CONTEXT: 'select[name="context"], select:has(option:text-matches("électoral", "i"))',
  MESSAGE: 'textarea[name="message"], textarea[placeholder*="message"]',
  SUBMIT: 'button[type="submit"]:has-text("Envoyer"), button:has-text("Demander un devis")',
} as const
...
export const TestTimeouts = {
  ...
  NETWORK_LOAD: 15000, // 15 secondes pour les chargements réseau
 ...
 */
export async function waitForClickableElement(page: any, selector: string, timeout = TestTimeouts.DEFAULT_ASSERTION) {
 ...
 */
export async function fillFormSafely(page: any, formData: Record<string, string>) {
  for (const [field, value] of Object.entries(formData)) {
    ...
    if (selector) {
      ...
      if (currentValue !== value) {
        throw new Error(`Failed to fill field ${field}. Expected: ${value}, Got: ${currentValue}`)
      }
    }
  }
}
 ...
 */
export async function setNetworkConditions(page: any, conditions: typeof NetworkConditions[keyof typeof NetworkConditions]) {
